// Perek35.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CPerek35, CPerek3DBase)
...
BEGIN_MESSAGE_MAP(CPerek35, CPerek3DBase)
	ON_UPDATE_COMMAND_UI_RANGE(ID_P35_1, ID_P35_1, &CPerek2DBase::OnSubHeadUpdate)
	ON_COMMAND_RANGE(ID_P35_1, ID_P35_1, &CPerek3DBase::OnSubHeadBtn)
	ON_WM_CREATE()
	ON_WM_MOUSEMOVE()
END_MESSAGE_MAP()
...
// CPerek35 diagnostics

#ifdef _DEBUG
void CPerek35::AssertValid() const
{
	CView::AssertValid();
}

#ifndef _WIN32_WCE
void CPerek35::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}
#endif
...
int CPerek35::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_slider.SetCaption("גובה השמש");
	m_slider.SetRefPoint(bottom);
	m_slider.SetMinMax(23.5, 113.5);
...
void CPerek35::Do3DPaint(void)
{
	...
	double AngNetyatHaMazalot = asin(sinNetya) * ToD;
	m_3dscreen.BeginObject(arez);
	{
		...
		m_3dscreen.TurnShape(m_xTurn- 90.0 - AngNetyatHaMazalot, axis_X, false);
	...
	m_3dscreen.BeginObject(ofanHaziHaYom);
	{
		double govaShemes = (/*-angNetya + */m_xTurn);
		...
		CFillRectData data;
		data.vec = range;
		m_3dscreen.FillRect(0,0,trszSteps,3, colorShemesh, 
	...
	m_3dscreen.ResetTranslation();
	m_3dscreen.ResetShift();
	m_3dscreen.TurnObjectCenter(m_xTurn-90, axis_X);
	...
	m_3dscreen.TurnShape(m_xTurn - 90 - AngNetyatHaMazalot, axis_X,false);
	m_3dscreen.TurnShape(m_yTurn, axis_Y, false);
	m_3dscreen.TurnShape(m_xStart, axis_X, false);
	m_3dscreen.FillRect(0,0,trszSteps,3, 0,fr_zIntensityAll, data2);
	m_3dscreen.TurnShape(0.0, axis_X);
...
void CPerek35::PaintResult(void)
{
	...
	CString sTxt;
	COLORREF txtcolor = m_memDc.SetTextColor(colorShemesh);
	sTxt.Format("%s\n%4.1f","גובה השמש מהאופק הדרומי" ,m_xTurn);
	...
	sTxt.Format("%s\n%4.1f","חיסור נטיית גלגל המזלות מגובה השמש" ,angNetya);
	nTxtHeight = m_memDc.DrawText(sTxt, rc2, 
		DT_RIGHT | DT_WORDBREAK | DT_TOP /*| DT_RTLREADING*/);

	double rohav = 90 - m_xTurn + angNetya;
	CString sRohav;
	if(rohav > 0.09)
		sRohav = "צפוני";
	else if(rohav < -0.09)
...
void CPerek35::OnMouseMove(UINT nFlags, CPoint point)
{
	CClientDC dc(this);
	OnPrepareDC(&dc);
	CPoint p = point;
...
void CPerek35::InitOnSubHeadButton(void)
{
	lasty = m_yTurn = m_yStart;
...
BOOL CPerek35::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	switch(message)
	{
	case WM_EXTRA_DRAW:
...
void CPerek35::PaintSlider(void)
{
	CChapPage &cp = theApp.m_chaptersParagraphs.m_chapPageArr[m_IDX_PEREK];
