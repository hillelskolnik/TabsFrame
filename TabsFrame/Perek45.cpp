// Perek45.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CPerek45, CPerek3DBase)
...
BEGIN_MESSAGE_MAP(CPerek45, CPerek3DBase)
	ON_UPDATE_COMMAND_UI_RANGE(ID_P45_1, ID_P45_1, &CPerek2DBase::OnSubHeadUpdate)
	ON_COMMAND_RANGE(ID_P45_1, ID_P45_1, &CPerek3DBase::OnSubHeadBtn)
	ON_WM_CREATE()
	ON_WM_MOUSEMOVE()
END_MESSAGE_MAP()
...
// CPerek45 diagnostics

#ifdef _DEBUG
void CPerek45::AssertValid() const
{
	CView::AssertValid();
}

#ifndef _WIN32_WCE
void CPerek45::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}
#endif
...
int CPerek45::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CPerek45::Do3DPaint(void)
{
	...
	double maslulHaShemesh = (m_xTurn - 90.0);
	...
	m_3dscreen.FillRect(stepsEmzai,8,stepMenat,6, color0, fr_paintTranslated);
	if((stepsEmzai + stepMenat) > 1080)
	{
		stepMenat = (stepsEmzai + stepMenat) - 1080;
		m_3dscreen.FillRect(0,8,stepMenat,6, color0, fr_paintTranslated);
	}
...
void CPerek45::OnMouseMove(UINT nFlags, CPoint point)
{
	CClientDC dc(this);
	OnPrepareDC(&dc);
	CPoint p = point;
...
BOOL CPerek45::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	switch(message)
	{
	case WM_EXTRA_DRAW:
...
void CPerek45::PaintSlider(void)
{
	m_slider.InitOnSubHeadButton();
	m_slider2.InitOnSubHeadButton();
}

void CPerek45::PaintResult(void)
{
	CClientDC dc(this);
	OnPrepareDC(&dc);
...
void CPerek45::InitOnSubHeadButton(void)
{
	PaintSlider();
}
